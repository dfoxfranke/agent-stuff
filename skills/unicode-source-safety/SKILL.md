...
description: "Determine whether literal non-ASCII Unicode text can be written reliably in an exact program-source or documentation-source file across every supported reader, compiler, interpreter, formatter, renderer, build, documentation, and code-generation path. Use this skill when writing or review requires a source-encoding and transport decision for Unicode in comments, documentation comments, standalone documentation, literals, identifiers, or other source regions; do not use it to decide grammar, semantics, style, escaping, normalization, or Unicode security."
---
...
whether the programming language or documentation grammar permits the
characters in the intended location and whether their use is semantically
correct and consistent with project policy. Do not infer that Unicode is
appropriate in identifiers, string or protocol values, regular expressions,
directives, or generated output. Treat normalization, bidirectional text,
confusables, and other Unicode security or style concerns as separate
decisions.
...
the [RedMonk January 2025 top 20](https://redmonk.com/sogrady/2025/06/18/language-rankings-1-25/)
and additional common or instructive cases. Apply more-specific file and
toolchain evidence before producing the final result.
