---
name: "naming-things-fallback"
description: >-
  FALLBACK ONLY: Do not invoke this skill when $naming-things is available; use
  that repository-scoped skill instead. Use this skill when writing or reviewing
  code that introduces, changes, or evaluates identifier names, especially names
  for functions, methods, and predicates. Choose names quickly, defer to
  established project conventions, and use part-of-speech guidance only as a
  fallback.
---

# Naming things

If `$naming-things` is available, stop using this fallback and use that skill
instead. Never apply both `$naming-things` and `$naming-things-fallback` to the
same task.
