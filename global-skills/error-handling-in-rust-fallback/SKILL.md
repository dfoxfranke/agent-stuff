---
name: "error-handling-in-rust-fallback"
description: >-
  FALLBACK ONLY: Do not invoke this skill when $error-handling-in-rust is
  available; use that repository-scoped skill instead. Use this skill when
  adding, changing, or specifically reviewing Rust error handling, including
  API design, choosing between errors and panics, selecting structured or
  open-ended error types, writing panic-site messages, and preserving error
  source chains.
---
...
Before adding, changing, or reviewing error handling, determine whether the
repository imposes any constraints relevant to the affected code.

Use this precedence order:

1.  **Explicit repository instructions.** Follow applicable `AGENTS.md`,
    contributor documentation, style guides, and documented API or panic
    policies.
2.  **Compatibility requirements.** Inspect the affected error types and
    signatures, especially public ones. Trace the call sites touched by the task
    and direct repository uses of any public variant, field, source link, or
    rendered behavior the change would alter. Stop at the first handler or
    presenter on each path.
3.  **Existing analogous code.** If a relevant choice remains open, inspect the
    two or three nearest comparable cases in the same module or crate. Stop when
    they establish a consistent precedent or show that local practice is mixed.

Do not infer a repository-wide convention from one isolated call site or
inventory every error type and crate in use.

Carry forward only constraints that affect the current change. These may include
a panic policy stricter than the default below or behavior on the affected call
paths that must remain compatible, such as matching a variant, reading a field,
traversing a source chain, or relying on rendered output.

If no relevant constraint decides a question, apply the core rules below. Do not
treat existing code as permission to violate an unconditional core rule.
...
Use `$api-documentation-quality-in-rust` when available; otherwise use
...
poisoning represents a bug under the applicable panic policy, normally write:
...
source's `Display` implementation. Unless an established codebase convention
clearly says otherwise, assume that the reporting boundary traverses source
chains.
...
1.  Read applicable repository instructions. Trace the call sites touched by the
    task and direct repository uses of any public error behavior the change
    would alter, stopping at the first handler or presenter on each path. Record
    only behavior the change must preserve. If a relevant choice remains open,
    inspect two or three nearby analogues and stop once they establish a
...
When reviewing Rust error handling, first perform the same task-local inspection
an author should have performed.
...
- uses `Box<dyn Error>` or a similar erased trait object for an open-ended
  extension API where the established or otherwise applicable choice is
  `anyhow::Error`;
...
exposes unstable implementation detail through `Debug`.
