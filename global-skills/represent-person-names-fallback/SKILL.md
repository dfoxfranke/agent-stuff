---
name: "represent-person-names-fallback"
description: >-
  FALLBACK ONLY: Do not invoke this skill when $represent-person-names is
  available; use that repository-scoped skill instead. Represent the names
  of people faithfully in standalone documentation, ordinary source-code
  comments, and documentation comments. Use this skill when writing or
  reviewing prose that names a person, especially when choosing preferred
...
enforced requirements, but do not search neighboring files or the wider codebase
merely to discover an implicit naming convention. If the target file is mixed or
provides no clear precedent, use the defaults below.
...
1.  Use `$unicode-source-safety` when available; otherwise use
    `$unicode-source-safety-fallback`. Never apply both. Apply the selected
    skill to the exact target file and every supported source-processing path.
...
Re-read the rendered or displayed prose when possible. Confirm that the name has
the intended spelling, that a non-Latin name follows the first-mention rule, and
that no source representation accidentally became part of the visible name.
