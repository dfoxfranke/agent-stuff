...
description: >-
  Use this skill when writing or reviewing Rust code that emits log messages or
  `tracing` events, including choosing frameworks and levels, placing events,
  defining structured fields and names, handling authentication events,
  and controlling log volume. Do not use it for log-handling infrastructure
  that only processes messages and emits none of its own.
---
...
Use `tracing` and its ecosystem for every new first-party component. When an
existing component already uses another logging framework, do not introduce a
parallel framework through an isolated edit. Continue using that component's
framework for in-scope events unless the task explicitly includes a framework
migration; do not broaden an ordinary logging change into such a migration.
Treat dependency, feature, subscriber, logger, and build configuration as hard
integration constraints.

Decide event names, fields, and severity independently from the framework. Name
higher-level events when they need stable identities for centralized collection,
queries, aggregation, alerts, or other programmatic use; otherwise omit explicit
names. Reuse semantic field keys for the same concepts and apply the level rules
below from the emitter's operational perspective.
...
1.  Classify the target as first-party code or vendored-fork code. For a
    vendored fork, apply the convention map from `$work-in-vendored-forks`. Use
    `tracing` for a new first-party component; retain an existing component's
    framework unless framework migration is explicitly in scope. Choose its
    event-name policy from how the events are consumed.
...
When reviewing Rust logging, first perform the same target classification,
framework-scope check, and contextual event-name, field, and severity decisions
an author should have performed.

Then report a finding when code:

- uses a framework other than `tracing` for a new first-party component,
  introduces a parallel framework through an isolated edit, or migrates an
  existing component without explicit task scope;
- applies an explicit-event-name policy inconsistent with the component's
...
- Use `tracing` for new components; retain an existing component's framework
